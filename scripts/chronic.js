...
function getUserListData() {
    const apiUrl = 'https://pharos3.ewj100.com/productStockUserCount/getPatientUser';
    const params = new URLSearchParams({
        airTrainMacAdd: "40:24:b2:a9:e4:6f",
        balanceMacAdd: "78:f2:35:05:c6:07"
...
function loadRowUser(user, index) {
    ...
    editButton.textContent = '查看记录';
    editButton.className = 'btn btn-primary btn-sm'; // 应用样式类名
    editButton.dataset.toggle = 'modal';
    editButton.dataset.target = '#recordsModal';
...
function viewRecord(idCard, button) {
    // idCard = '411627199208258213'
    dateList = []
    loadList = []
    painLevelList = []
    warningTimeList = []
    ...
    // const apiUrl = 'https://api.jhxiao-school.com/planUserTrainRecord/getTrainRecord';
    const apiUrl = 'https://pharos3.ewj100.com/planUserTrainRecord/getTrainRecord';
    const params = new URLSearchParams({
        idCard: idCard,
    });
    // 将参数拼接在URL的末尾
        ...
        if (data.code == 0) {
            ...
            if (data.data) {
                data.data.map(item => {
                    ...
                    records = records.concat(item.tbUserTrainRecordList)
                    console.log('训练记录data--->', records)
                })
                // records = data.data.tbUserTrainRecords
            } else {
                return
            }
...
function showSubChart(userId, frequency, dateStr) {
    ...
    // const apiUrl = 'https://api.jhxiao-school.com/planUserTrainRecord/getByUserIdAndFrequency';
    const params = new URLSearchParams({
        userId: userId,
        frequency: frequency,
        dateStr: dateStr
...
// // 当分页改变时更新分页按钮状态
// $('#paginationContainer').on('pageChanged', function (event, newPage) {
//   currentPage = newPage;
